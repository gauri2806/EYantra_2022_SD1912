...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [36.66,30.12,48.48]  #throttle = 20.64ss
		self.Ki = [0,0,0]
		self.Kd = [864.6,823.8,626.7]      #throttle = 720.3
	...
	#	2. Compute the error (for proportiona	l), change in error (for derivative) and sum of errors (for integral) in each axis. Refer "Understanding PID.pdf" to understand PID equation.
	...
	#	8. Add error_sum
		...
		self.cmd.rcPitch = int(1500 + self.pitch_error * self.Kp[1] + (self.pitch_error - self.pitch_prev_error) * self.Kd[1] + self.pitch_sum_error * self.Ki[2])
